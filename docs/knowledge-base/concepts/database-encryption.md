...
Explain what `T9_DB_KEY` actually protects, and what it does not.

## Two layers

1. **Whole-file seal.** Persistent path `T9_DATA/t9.db.sealed` = magic `T9DB1\n` + AES-256-GCM of the SQLite file bytes. File key = HKDF-SHA256(`T9_DB_KEY`, info `t9-db-file-v1`). AAD is the magic string.
...
- Optional `T9_RESET_DB` deletes sealed file + `t9.db.keyfp`
- Decrypts sealed bytes into `.t9.work.db` (mode 0600)
...
`t9.db.keyfp` stores that fingerprint so a wrong key can be diagnosed as mismatch vs corruption.
