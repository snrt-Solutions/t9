...
Show public health and capability flags so an operator (or a tunnel check) can confirm `aesmsd` is up without offering a login.
...
- Info includes `name: "aesms"`, `fingerprint`, `base_url`, `message_ttl_h: 24`, `max_graphemes: 160`, `web_login: false`, `pii: false`, `fetch_once: true`, `one_device: true`.
- No cookies.

## Edge Cases

- If `AESMS_BASE_URL` is wrong, status still loads locally but `release_url` values from device login will point at the misconfigured origin.
